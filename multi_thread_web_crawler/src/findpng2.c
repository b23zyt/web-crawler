...
/*
Reference: Linux hsearch: https://man7.org/linux/man-pages/man3/hsearch.3.html
*/
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
// linked list implementation of the unbounded queue
typedef struct Node{
    char* url;
    struct Node* next;
} Node;
...
void process_html(CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    ...
    find_http(p_recv_buf->buf, p_recv_buf->size, follow_relative_link, url); //enqueue inside this!
    //sprintf(fname, "./output_%d.html", pid);
    //return write_file(fname, p_recv_buf->buf, p_recv_buf->size); //write the html to a file
}
...
void process_png(CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    ...
    if(is_png((unsigned char *)p_recv_buf->buf, p_recv_buf->size) == 0){
        ...
        char n_eurl[1024];
        sprintf(n_eurl, "%s\n", eurl);
        int pre_count = atomic_fetch_add(&curr_png, 1);
        if(pre_count < max_png){
            pthread_mutex_lock(&file_lock);
            if(pre_count == max_png - 1){
        ...
        if(pre_count >= max_png - 1){ //set when pre_count = 19
            atomic_store(&done, 1);
            sem_post(&filled);
        }
    }
...
void process_data(CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    ...
    //https://curl.se/libcurl/c/curl_easy_getinfo.html: extract information from a curl handler
...
void* do_work(void *arg){
    while(1){
        ...
        if (atomic_load(&count) == 0) {
            int idle_time = 0;
            while (atomic_load(&count) == 0 && idle_time < 500000) { // 0.5s
                usleep(50000);
                idle_time += 50000;
            }
            if (idle_time >= 500000 && atomic_load(&count) == 0) {  
                break;
            }
        }
        ...
        process_data(curl_handle, &recv_buf); //enqueue at here
...
{
    int c;
    int t = 1; //number of threads (default = 1)
    ...
    atomic_store(&curr_png, 0);
    atomic_store(&done, 0);
    pthread_mutex_init(&lock, NULL);
    pthread_mutex_init(&file_lock, NULL);
    pthread_mutex_init(&visited_lock, NULL);
    sem_init(&filled, 0, 0);
    hcreate_r(10000, &visited); //assume maximum 10000 urls;
    ...
    for(int i = 0; i < t; i++){
        pthread_create(&p_tids[i], NULL, do_work, NULL);
    }
    ...
    printf("findpng2 execution time: %.2f seconds\n", times[1] - times[0]);
    
    //free the linked list from head to tail
    while(head != NULL){
        Node* temp = head;
        head = head->next;
        free(temp->url);
